...
/*
 *
Adapted tga class from tinyrenderer tutorial since my 
tga class is a mess
i'm such a retard for trying to do that
still i might try to implement the tga class myself 
but not now...
 *
 */
#include <fstream>
...
public:
	...
	TGA();
	TGA(const char *fname,int w, int h, int bpp);
	TGA(const TGA &img);
	bool readFile(const char *filename);
	bool writeFile(bool rle);
...
TGA::TGA(const char *fname,int w, int h, int bpp=4) : data(NULL), width(w), height(h), bytespp(bpp) {
...
bool TGA::writeFile(bool rle=false) {
...
bool TGA::setPixel(int x, int y, COLOR c) {
	if (!data || x<0 || y<0 || x>=width || y>=height) {
		return false;
	}
